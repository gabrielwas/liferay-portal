...
public class ObjectLayoutBox implements Serializable {
	...
	@Schema
	@Valid
	public ObjectLayoutBoxRow[] getObjectLayoutBoxRows() {
		return objectLayoutBoxRows;
	}

	public void setObjectLayoutBoxRows(
		ObjectLayoutBoxRow[] objectLayoutBoxRows) {

		this.objectLayoutBoxRows = objectLayoutBoxRows;
	}

	@JsonIgnore
	public void setObjectLayoutBoxRows(
		UnsafeSupplier<ObjectLayoutBoxRow[], Exception>
			objectLayoutBoxRowsUnsafeSupplier) {

		try {
			objectLayoutBoxRows = objectLayoutBoxRowsUnsafeSupplier.get();
		}
		catch (RuntimeException re) {
			throw re;
		}
	...
	@GraphQLField
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	protected ObjectLayoutBoxRow[] objectLayoutBoxRows;
	...
	public String toString() {
		...
		if (objectLayoutBoxRows != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}

			sb.append("\"objectLayoutBoxRows\": ");

			sb.append("[");

			for (int i = 0; i < objectLayoutBoxRows.length; i++) {
				sb.append(String.valueOf(objectLayoutBoxRows[i]));

				if ((i + 1) < objectLayoutBoxRows.length) {
					sb.append(", ");
				}
			}
