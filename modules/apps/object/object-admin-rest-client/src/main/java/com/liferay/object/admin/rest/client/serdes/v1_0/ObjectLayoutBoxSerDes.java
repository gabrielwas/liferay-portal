...
public class ObjectLayoutBoxSerDes {
	...
	public static String toJSON(ObjectLayoutBox objectLayoutBox) {
		...
		if (objectLayoutBox.getObjectLayoutBoxRows() != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < objectLayoutBox.getObjectLayoutBoxRows().length;
				 i++) {

				sb.append(
					String.valueOf(
						objectLayoutBox.getObjectLayoutBoxRows()[i]));

				if ((i + 1) < objectLayoutBox.getObjectLayoutBoxRows().length) {
					sb.append(", ");
				}
			}
	...
	public static Map<String, String> toMap(ObjectLayoutBox objectLayoutBox) {
		...
		if (objectLayoutBox.getObjectLayoutBoxRows() == null) {
			map.put("objectLayoutBoxRows", null);
		}
		else {
			map.put(
				"objectLayoutBoxRows",
				String.valueOf(objectLayoutBox.getObjectLayoutBoxRows()));
		}
			...
			if (Objects.equals(jsonParserFieldName, "collapsable")) {
				...
				if (jsonParserFieldValue != null) {
					objectLayoutBox.setName(
						...
						jsonParserFieldName, "objectLayoutBoxRows")) {

				if (jsonParserFieldValue != null) {
					objectLayoutBox.setObjectLayoutBoxRows(
						Stream.of(
							toStrings((Object[])jsonParserFieldValue)
						).map(
							object -> ObjectLayoutBoxRowSerDes.toDTO(
								(String)object)
						).toArray(
							size -> new ObjectLayoutBoxRow[size]
						));
				}
			}
