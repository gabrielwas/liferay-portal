...
public class ObjectFilterUtil {
	...
	public static List<String> getODataObjectFilters(
		List<ObjectFilter> objectFilters) {

		List<String> oDataObjectFilters = new ArrayList<>();

		if (objectFilters == null) {
			return oDataObjectFilters;
		}
		...
		for (ObjectFilter objectFilter : objectFilters) {
			...
			for (String operator : operators) {
				...
				if (object instanceof Object[]) {
					String[] transform = TransformUtil.transform(
						(Object[])object,
					...
					StringBundler sb = new StringBundler(transform.length + 2);

					sb.append(StringPool.OPEN_PARENTHESIS);
					sb.append(ArrayUtil.toString(transform, StringPool.BLANK));
					sb.append(StringPool.CLOSE_PARENTHESIS);

					object = sb.toString();
				}

				oDataObjectFilters.add(
