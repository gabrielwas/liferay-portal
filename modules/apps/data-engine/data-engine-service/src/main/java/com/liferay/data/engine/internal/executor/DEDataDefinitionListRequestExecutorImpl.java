 ...
 */
@Component(immediate = true, service = DEDataDefinitionListRequestExecutor.class)
public class DEDataDefinitionListRequestExecutorImpl implements DEDataDefinitionListRequestExecutor {
	...
	public DEDataDefinitionListResponse execute(
		...
		try {
			...
			List<DDMStructure> ddmStructures = ddmStructureLocalService.getStructures(deDataDefinitionGroupId);
			...
			ddmStructures.forEach(ddmStructure -> {
				...
				} catch (DEDataDefinitionFieldsDeserializerException e) {
					e.printStackTrace();
				}
			});
