...
| `CRAWLER_CRAWL_RULES_EXTRA_YAML` | *(empty)* | Per-data-source `crawl_rules` **list items** (not the `crawl_rules:` key itself), indented 12 spaces to sit under the base rules. Generated by the Spring Boot dispatcher from the content retriever's Include/Exclude Paths. Appended **after** the base hygiene rules, because the crawler evaluates rules top-down, first-match-wins: base denies, then per-source excludes, then include allows, then a terminal `deny .*` that turns the include list into a whitelist. When empty the base rules apply alone. |
| `CRAWLER_MAX_CRAWL_DEPTH` | `10` | `max_crawl_depth`. How many link-levels from the seed URL to follow. Default matches the crawler's own built-in default, so leaving it unset preserves pre-existing behavior. |
| `CRAWLER_MAX_EXTRACTED_LINKS_COUNT` | `1000` | `max_extracted_links_count` — how many links the crawler follows from a single page. Not to be confused with `max_indexed_links_count`, which is a per-document field-size limit. |
| `CRAWLER_MAX_UNIQUE_URL_COUNT` | `100000` | `max_unique_url_count`. The crawl stops once this many unique pages have been discovered across the site. |
