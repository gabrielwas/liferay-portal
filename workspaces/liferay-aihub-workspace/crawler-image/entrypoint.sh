...
# Per data source crawl limits. Defaults match the crawler's own built-in
# defaults, so an unset value leaves behavior exactly as it was before these
# knobs existed.
: "${CRAWLER_MAX_CRAWL_DEPTH:=10}"
: "${CRAWLER_MAX_EXTRACTED_LINKS_COUNT:=1000}"
: "${CRAWLER_MAX_UNIQUE_URL_COUNT:=100000}"
# Optional per data source crawl_rules list items, generated by the Spring Boot
# dispatcher from the content retriever's include/exclude path patterns. Appended
# to the base rules below, so the include whitelist's terminal deny lands last.
...
# Append the per data source rules. Order is load bearing: the crawler evaluates
# rules top down, first match wins, so the base hygiene denies stay ahead of the
# per source excludes, which stay ahead of the include allows and their terminal
# deny.
if [[ -n "${CRAWLER_CRAWL_RULES_EXTRA_YAML}" ]]; then
	CRAWL_RULES_YAML="${CRAWL_RULES_YAML}
${CRAWLER_CRAWL_RULES_EXTRA_YAML}"
fi
...
max_extracted_links_count: ${CRAWLER_MAX_EXTRACTED_LINKS_COUNT}
max_unique_url_count: ${CRAWLER_MAX_UNIQUE_URL_COUNT}
output_sink: file
